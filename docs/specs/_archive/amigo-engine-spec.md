...
Sprite animations from Aseprite tags. Frame-based with per-frame duration (fixed-point). Looping/one-shot modes. Phase 2: skeletal animation for large bosses.
...
Three MCP servers side by side: `amigo` for engine control, `amigo-artgen` for art (see amigo-artgen-spec.md), `amigo-audiogen` for audio (see amigo-audiogen-spec.md).
...
// → {"result": {"path": "/tmp/frame.png", "width": 640, "height": 360}}
...
F2: Grid, F3: Collision boxes, F4: Pathfinding, F5: Spawn/build zones, F6: Performance, F7: Entity list, F8: Network stats.

### Dev Mode Features

- Hot reload (assets + data)
- State snapshot to file
- Tick stepping (one tick at a time)
...
Engine subsystems are structured as plugins with compile-time feature flags for modularity.
...
No dynamic plugin loading, no runtime discovery. Compile-time decides what's included, Plugin Trait provides clean initialization order and update lifecycle.
...
Overridable via CLI flags (`amigo run --fullscreen`). Saved when player changes settings.

---

## 24. Game-Specific Design

Game design and asset pipeline decisions are maintained in separate spec files:

- **Amigo TD**: See `amigo-td-spec.md` (worlds, targeting, pathfinding, atmosphere)
- **TD UI/UX**: See `amigo-td-ui-spec.md` (HUD, radial menus, screens, gamepad, world-themed frames)
- **Art Pipeline**: See `amigo-artgen-spec.md` (ComfyUI integration, post-processing, style definitions)
- **Audio Pipeline**: See `amigo-audiogen-spec.md` (ACE-Step music gen, AudioGen SFX, adaptive music system, stem-based vertical layering)
...
| Virtual Resolution | 640×360, Integer Scaling (3×/4×/6×) | Pixel-perfect auf 1080p/1440p/4K, Ultrawide erweitert horizontal |
